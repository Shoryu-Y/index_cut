...
class IndexCutPainter2 extends CustomPainter {
  const IndexCutPainter2({
    required this.itemCount,
    required this.currentIndex,
    required this.isOneWay,
    required this.height,
  ...
  void paint(Canvas canvas, Size size) {
    ...
    for (var index = 0; index < itemCount; index++) {
      if (index < currentIndex && !isOneWay) {
        final end = size.width * (index + 1) / itemCount;
        final rect = Rect.fromPoints(
          Offset(end - curveRadius * 2, size.height - height),
          Offset(end, size.height - (height - curveRadius * 2)),
        );
        final path = Path()
          ..moveTo(0, size.height)
          ..lineTo(0, size.height - height)
          ..lineTo(end - curveRadius, size.height - height)
          ..lineTo(end, size.height - (height - curveRadius))
          ..lineTo(end, size.height)
          ..close()
          ..addArc(rect, pi * 3 / 2, pi / 2);
        canvas.drawShadow(path, shadowColor, 1, true);
      }
      ...
      if (index > currentIndex) {
        final start = size.width * index / itemCount;
        final rect = Rect.fromPoints(
          Offset(start, size.height - (height - curveRadius * 2)),
          Offset(start + curveRadius * 2, size.height - height),
        );
        ...
        final path = Path()
          ...
          ..lineTo(start + curveRadius, size.height - height)
          ..lineTo(start, size.height - (height - curveRadius))
          ..close()
          ..addArc(rect, pi, pi / 2);
      ...
      if (index == currentIndex) {
        ...
        final leftRect = Rect.fromPoints(
          ...
          Offset(start + curveRadius * 2, size.height - (curveRadius * 2)),
        );
        final rightRect = Rect.fromPoints(
          Offset(end, size.height - 0),
          Offset(end - 2 * curveRadius, size.height - (2 * curveRadius)),
        );
        ...
        if (!isOneWay) {
          path
            ..arcTo(leftRect, pi / 2, pi / 2, true)
            ..moveTo(start, size.height - 0)
            ..lineTo(start, size.height - curveRadius)
            ..lineTo(start + curveRadius, size.height - 0)
            ..close();
        }

        path
          ..arcTo(rightRect, 0, pi / 2, true)
          ..moveTo(end, size.height - 0)
          ..lineTo(end, size.height - curveRadius)
          ..lineTo(end - curveRadius, size.height - 0);
  ...
  @override
  bool shouldRepaint(IndexCutPainter2 oldDelegate) {
    return true;
  }
}
